#!/bin/bash
# Ralph loop for the app/native branch. Usage: ./ralph.sh [max_iterations]
...
[ -f "$DIR/progress.txt" ] || printf '# Ralph progress (app/native)\nStarted: %s\n---\n' "$(date)" > "$DIR/progress.txt"
for i in $(seq 1 "$MAX"); do
  ...
  if echo "$OUT" | grep -q "<promise>WAITING_SIMULATOR</promise>"; then echo "waiting for simulator runtime (5 min)"; sleep 300; continue; fi
  sleep 3
done
