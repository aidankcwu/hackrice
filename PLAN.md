...
- [x] F.4 (web-builder) Settings. Voice toggle (localStorage, labelled "Speak through the glasses"), Wind‑down time (`PUT /api/persona` if Job B exposed a field, else local and labelled "not connected yet" in muted text), About with version and backend URL.
