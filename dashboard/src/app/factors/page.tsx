...
/** Keep `?goal=` when moving between the two pages. */
function withParams(path: string, goal: Goal): string {
  return goal === "average" ? path : `${path}?goal=${encodeURIComponent(goal)}`;
}
...
function EngineDown({ error, goal }: { error: unknown; goal: Goal }) {
      ...
      <main className="mx-auto w-full max-w-6xl px-6 py-10">
        <section className="rounded-panel bg-surface p-6 md:p-8" aria-labelledby="engine-down-title">
          <h1 id="engine-down-title" className="text-2xl font-bold text-ink">
            No measured day to list
          </h1>
          <p className="mt-2 text-muted">
            This page lists the factors behind today&apos;s score, every one of them read from the backend and scored by
            the engine. With either unreachable there is nothing measured to list, and nothing is filled in for it.
          </p>
...
export default async function FactorsPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  try {
    data = await loadDashboardData({ goal });
  } catch (error) {
    return <EngineDown error={error} goal={goal} />;
  }
