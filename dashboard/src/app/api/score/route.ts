 ...
 * Windows too.
 *
 * `GET /api/score?goal=<Goal>&mock=1` — the whole `DashboardData` the page
 * renders, for the client-side poll.
 */
...
export async function GET(req: Request): Promise<NextResponse> {
  ...
  const mock = mockParam === null ? undefined : mockParam === "1" || mockParam === "true";
  try {
    return json(await loadDashboardData({ goal: resolveGoal(params.get("goal")), mock }));
  } catch (e) {
