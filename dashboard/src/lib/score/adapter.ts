 ...
 *
 * Data honesty: nothing here is invented. A field the pipeline does not
 * measure is left undefined so the engine imputes the population reference
 * and marks the factor unmeasured. The one deliberate stretch (every gym
 * minute counted as strength) is flagged where it happens.
 */
...
/** backend/pipeline/models.py HEALTHY_FOOD_TYPES — what the pipeline itself calls on-pattern. */
const HEALTHY_FOOD_TYPES: ReadonlySet<string> = new Set([
  "vegetables", "fruit", "grains", "fish", "poultry", "salad", "rice_bowl", "soup", "eggs", "nuts", "mixed",
]);
...
export function episodeToEngine(episode: PipelineEpisode, ctx: { nowT: number }): EngineEpisode | null {
  ...
  if (type === "conversation") {
    ...
    const food = String(episode.dominant.food_type ?? "meal");
    out.label = food;
    out.tags = HEALTHY_FOOD_TYPES.has(food) ? [food, "mediterranean"] : [food];
...
/** Habitual bedtime (decimal hours, may exceed 24) from a week of seeded `bed_time` rows; 23 when unknown. */
 ...
 *
 * Not set, because nothing in the pipeline measures them: `night_lux`
 * (lux_proxy is not lux), `vo2max_pct`, `purpose`, `height_m`, `smoker`.
 */
