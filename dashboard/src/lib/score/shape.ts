 ...
 */
export function forecastView(payload: EnginePayload, bedtime_hh: number): ForecastView {
  const forecast = payload.forecast;
  const drivers = forecast.drivers;
  const bedtime = hhmm(bedtime_hh);
  ...
  if (shift) sentences.push(`Lights out at ${bedtime} keeps your regularity score.`);
  if (drivers.length === 0) {
    sentences.push(`Nothing is dragging tonight down. Lights out near ${bedtime} keeps it that way.`);
  }
...
const average = (values: Array<number | null>): number | undefined => {
...
export function weekSummary(week: WeekDay[]): string {
  ...
  if (late.length > 0 && rest.length > 0 && lateSleep !== undefined && restSleep !== undefined) {
    const lateRec = average(late.map((d) => d.rec));
    const restRec = average(rest.map((d) => d.rec));
    const lateText =
      `${late.length} late-caffeine day${late.length === 1 ? "" : "s"} averaged ${lateSleep.toFixed(1)} h of sleep` +
      (lateRec === undefined ? "" : ` and recovery ${Math.round(lateRec)}`);
    const restText =
      `the other ${rest.length} averaged ${restSleep.toFixed(1)} h` + (restRec === undefined ? "" : ` and ${Math.round(restRec)}`);
...
export function shapeDashboard({ payloads, days, person, source, engineMs }: ShapeArgs): DashboardData {
  ...
  return {
    ...
    pins: pinRows(today),
    forecast: forecastView(today, person.bedtime_hh),
