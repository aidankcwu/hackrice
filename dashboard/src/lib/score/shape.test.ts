...
const PERSON: Person = { name: "Lukas", age: 20, sex: "M", goal: "average", profileLabel: "Average", device: "Ray-Ban Meta + WHOOP", bedtime_hh: 23 };
...
describe("forecastView", () => {
  ...
  it("names a single booked driver and the bedtime shift", () => {
    ...
    const coffee = forecastView({ ...base, forecast: { ...base.forecast, drivers: ["caffeine at 16:00 is inside your 9 h cutoff"] } }, 23);
    expect(coffee.fix).toBe("The coffee is already booked.");
    const drinks = forecastView({ ...base, forecast: { ...base.forecast, drivers: ["1 drink(s) — expect a lower HRV tonight"] } }, 23);
    expect(drinks.fix).toBe("The drinks are already booked.");
    const shift = forecastView({ ...base, forecast: { ...base.forecast, drivers: ["bedtime +105 min vs habit"] } }, 24.75);
  ...
  it("says nothing is dragging when there are no drivers", () => {
    const fc = forecastView(payload({ observations: { night_screen_min: 0 } }), 23.5);
...
describe("weekSummary", () => {
  ...
  it("compares late-caffeine days with the rest and names the worst", () => {
    ...
    expect(text).toBe("2 late-caffeine days averaged 5.9 h of sleep and recovery 37; the other 2 averaged 7.5 h and 84. Mon was the worst: −0.9 h.");
  });
  ...
  it("drops recovery and the worst clause when they are missing", () => {
    ...
    expect(text).toBe("1 late-caffeine day averaged 5.8 h of sleep; the other 1 averaged 7.4 h.");
  });
...
describe("shapeDashboard", () => {
  it("assembles today's payload with the week and passes engine fields through", () => {
    const days = [day({ date: "2026-09-11", seeded: { sleep_hours: 7 }, nowT: at(10) }), day({ isToday: true, nowT: at(18, 30) })];
    ...
    const data = shapeDashboard({ payloads, days, person: PERSON, source, engineMs: 412 });
  ...
  it("refuses an empty run rather than inventing a day", () => {
    const source = { mode: "mock" as const, api_base: "", day: "2026-09-12" };
    expect(() => shapeDashboard({ payloads: [], days: [], person: PERSON, source, engineMs: 0 })).toThrow(/at least one payload/);
  });
});
