...
describe("allowedNumbers / sentenceIsSafe", () => {
  ...
  it("rejects a sentence with a number the annotation does not contain", () => {
    ...
    // A span length is not in the JSON, so it has to arrive as `extra`. The
    // span is 5 here, not the true 3, because `collect` already allows 3 as the
    // integer form of delta_hours −2.6 — a number the JSON does contain.
    expect(sentenceIsSafe("Thu→Sat: −2.6 h over 5 days.", run)).toBe(false);
    expect(sentenceIsSafe("Thu→Sat: −2.6 h over 5 days.", run, [5])).toBe(true);
  });
});
...
describe("narrate", () => {
  it("keeps a safe LLM sentence and falls back to the template on a leaked number or a failure", async () => {
    // The prompt is built from the annotation, so `"kind":"run"` is what the
    // stub matches on — there is no caller-supplied prompt to key off any more.
    const llm = async (prompt: string) => {
      if (prompt.includes('"kind":"run"')) return '"Thursday to Saturday you dropped 2.6 hours, with alcohol and night screens on every day."';
      if (prompt.includes('"kind":"contrast"')) return "Late caffeine nights averaged 6 h of sleep and about 1.5 h less than usual.";
      throw new Error("boom");
    };
    const out = await narrate([run, contrast, extreme], { days: DAYS }, llm);
  ...
  it("uses templates when no LLM is configured", async () => {
    const out = await narrate([extreme]);
    expect(out[0]).toMatchObject({ origin: "template" });
  });

  it("builds the prompt from the annotation and forbids inventing facts", () => {
    const prompt = narratorPrompt(run, { days: DAYS });
    expect(prompt).toContain(JSON.stringify(run));
    expect(prompt).toContain(templateSentence(run, { days: DAYS }));
    expect(prompt).toContain("introduce no number, date, metric or cause that is not already here");
  });
});
