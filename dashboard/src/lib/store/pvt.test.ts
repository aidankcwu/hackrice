...
describe("recordRun and forScoring", () => {
  it("stores runs, builds the baseline on the third test and exposes pvt + check for the day", async () => {
    ...
    expect(third.baseline).toMatchObject({ mu_rt: 323.3 });
    // means 300, 310, 360 → mu 323.3, SD 32.1 → z = (360 − 323.3) / 32.1
    expect(third.rt_z).toBeCloseTo(1.14, 2);
    expect(third.vs_baseline_ms).toBe(36.7);
