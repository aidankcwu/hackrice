# PLAN.md — Bryan iOS app on the existing pipeline
...
- [ ] 4.2 (ios-builder) `Sources/Actions/CalendarAct.swift`: EventKit write-only access (`NSCalendarsWriteOnlyAccessUsageDescription`), finds the next free 20 min between `earliest` and `latest` on the default calendar, inserts "Walk outside (Bryan)", replies `act_result`. Calendar row in Setup. `MacLink` routes `act` messages to an `ActRunner`.
  Skills: eventkit.
