...
- `/healthz`: process liveness only; the Fly health check uses it. Since the reconcile
  with the hosted-tester work (deploy/README.md), `/docs`, `/redoc` and `/openapi.json`
  are behind the token too. The gate is `backend/pipeline/api/auth.py`; `API_TOKEN` is
  read as an alias of its `ACCESS_TOKEN`, and `X-Access-Token` is accepted as well as
